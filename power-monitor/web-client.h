...
void ServeWebClients() 
{
  ...
  inString += F("<b>TEMPERATURE:</b> ");
  inString += temp;
  inString += F("<br>");
  inString += F("<table border=1px>");
  inString += F("<tr align=center><th>Sensor</th><th>Channel</th><th>Power</th><th>Total Sensor Power</th></tr>");
  int span=0;
  for (int i9 = 0; i9 < 10; i9++) {
    for (int i10 = 0; i10 < 3; i10++) {
        inString += F("</tr align=center>");
       ...
       inString += i10;
       inString += F("</td><td>");
       inString += valueSum[i9][i10];
       inString += F(" w");
       inString += F("</td>");
        
       if (span == 0){
       inString += F("<td rowspan='3'>");
       inString += valueSum[i9][3];
       inString += F("</td>");  
       span =1;
       }
       inString += F("</tr>");
  ...
  inString += F("</td></tr>");  
  inString += F("<tr><td><b>Baudrate:</b></td><td>");
  inString += CC_BAUD;
  inString += F("</td></tr>");  
  inString += F("</table><br />");
  inString += F("</div>");
